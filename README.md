...
- 源码扫描读取`src/`，Jeecg业务页面规则覆盖`src/views/`下所有层级，包括页面同目录的接口和表单配置文件；不要求名为`safe`的目录，也不按`system`、`monitor`等目录名自动排除。
- `src/components/`中的公共封装不适用业务页面的组件优先规则。Windows与macOS路径会统一为`/`形式，便于规则匹配和报告追踪。
- 静态检查是启发式风险提示，不是跨文件语义分析：发现缺少rowKey、重复提示等线索后，仍须核对外部配置、框架默认值及真实交互，不能直接据此删除业务代码。
- Test Agent按配置场景执行，不会自动推导完整业务。页面冒烟通过不代表业务验收通过；模拟写接口成功也不代表真实数据已保存。
- 推荐在业务项目内直接分析和开发，按需调用本工具；无需每个需求先跨对话生成一遍通用规则。
